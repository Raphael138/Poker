(**
    Test Plan:
    Which parts of the system were automatically tested by 
    OUnit vs manually tested?

    We automated the testing of our functions for ranking, sorting, and creating 
    decks using OUnit. Manually, we tested the game component of our system, to
    make sure the flow of the game in the REPL behaved properly. 

    What modules were tested by OUnit?

    We used OUnit to test our card module, in which all of the functions to rank 
    our decks and extract information about the community hands.

    How were test cases developed?

    - black box testing: for smaller helper methods (such as return string of 
    a card) we used black box testing to test the function based solely off the 
    specification that we had written.
    - glass box testing: For other more complex functions such as rank, we used 
    glass box testing to test all of the pattern matching that we implemented.

    Why does this testing approach demonstrate correctness of the system? 

    This testing approach demonstrates an overall correctness of our system 
    because we chose to automate both glass and black box testing for the 
    fundamental functions that enable us to manipulate the deck, while manually 
    simulating the game enviornment several times to ensure a smooth and correct
    progression of our rounds. 
*)
